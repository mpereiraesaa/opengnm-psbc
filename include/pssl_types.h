...
_Static_assert(sizeof(PsslBinaryParamInfo) == 0x1c, "");

typedef struct {
	uint8_t type;	   // PsslType
	uint8_t semantic;  // PsslSemantic
	uint8_t semanticindex;
	uint8_t resourceindex;

	uint8_t interptype;  // PsslFragmentInterpType
...
_Static_assert(sizeof(PsslBinaryAttribute) == 0x10, "");
...
typedef struct {
	...
	uint32_t stridesize;
	uint8_t buffertype;    // PsslBufferType
	uint8_t internaltype;  // PsslInternalBufferType
	uint8_t type;	       // PsslType
	uint8_t _unused;
...
_Static_assert(sizeof(PsslBinaryBuffer) == 0x18, "");

typedef struct {
	uint32_t elementoffset;
	uint32_t registeridx;
	uint8_t constantbufidx;
	uint8_t elementtype;  // PsslType
	uint8_t buffertype;   // PsslBufferType
	uint8_t _unused;
...
_Static_assert(sizeof(PsslBinaryConstant) == 0x20, "");
...
typedef struct {
	...
	uint8_t elementtype;  // PsslElementType
	uint32_t byteoffset;
	uint32_t size;
	uint32_t arraysize;
...
_Static_assert(sizeof(PsslBinaryElement) == 0x24, "");
...
_Static_assert(sizeof(PsslBinarySamplerState) == 0x10, "");
